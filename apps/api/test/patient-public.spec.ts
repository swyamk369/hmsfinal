...
describe('HMS public admin — gating & DTOs', () => {
  it('write endpoints require the right permissions', () => {
    ...
    expect(Reflect.getMetadata(PERMISSION_KEY, HmsPublicController.prototype.updatePortalSettings)).toContain(PERMISSIONS.PATIENT_PORTAL_SETTINGS_MANAGE);
    expect(Reflect.getMetadata(PERMISSION_KEY, HmsPublicController.prototype.createDoctorProfile)).toContain(PERMISSIONS.DOCTOR_PUBLIC_PROFILE_MANAGE);
  });
  ...
  it('hiding a profile requires a non-empty reason', async () => {
    ...
    expect((await validate(plainToInstance(HideReasonDto, { reason: 'No longer accepting online patients' }))).length).toBe(0);
  });
});
...
describe('Patient portal settings', () => {
  ...
  it('updating settings upserts and audits', async () => {
    ...
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'patient_portal.settings.update' }));
  });
});
...
describe('Public hospital profile', () => {
  it('creates a profile with a generated slug and syncs the search index', async () => {
    ...
    d.publicHospitalProfile.create.mockResolvedValue({ id: 'hp1', hospitalSlug: 'city-hospital-t1', hospitalDisplayName: 'City Hospital' });
    ...
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'public_profile.update' }));
  });
  ...
  it('publish requires an existing profile, sets PUBLISHED, syncs index, audits', async () => {
    ...
    d.publicHospitalProfile.update.mockResolvedValue({ id: 'hp1', isPublic: true, profileStatus: 'PUBLISHED', hospitalDisplayName: 'City' });
    await svc.publishHospitalProfile(ctx(d));
    expect(d.publicHospitalProfile.update).toHaveBeenCalledWith(expect.objectContaining({ data: { isPublic: true, profileStatus: 'PUBLISHED' } }));
    expect(index.syncHospital).toHaveBeenCalled();
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'public_profile.publish' }));
  });
});
...
describe('Public doctor profile', () => {
  it('rejects when the provider does not exist', async () => {
    ...
    await expect(svc.createDoctorProfile(ctx(d), { doctorId: '11111111-1111-4111-8111-111111111111', displayName: 'Dr A' } as any)).rejects.toBeInstanceOf(BadRequestException);
  });
  ...
  it('creates with a slug and audits; publish syncs the doctor into the index', async () => {
    ...
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'doctor_public_profile.create' }));

    d.publicDoctorProfile.findFirst.mockResolvedValue({ id: 'dp1', doctorId: 'doc1' });
    d.publicDoctorProfile.update.mockResolvedValue({ id: 'dp1', doctorId: 'doc1', isPublic: true, profileStatus: 'PUBLISHED' });
    ...
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'doctor_public_profile.publish' }));
  });
});
...
describe('Appointment types & availability', () => {
  it('creates an appointment type and audits', async () => {
    ...
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'appointment_type.create' }));
  });

  it('rejects an availability rule whose endTime is not after startTime', async () => {
    await expect(
      svc.createAvailabilityRule(ctx(d), { doctorId: 'doc1', dayOfWeek: 1, startTime: '17:00', endTime: '09:00' } as any),
    ).rejects.toBeInstanceOf(BadRequestException);
  });

  it('creates a valid availability rule and audits', async () => {
    d.availabilityRule.create.mockResolvedValue({ id: 'ar1' });
    await svc.createAvailabilityRule(ctx(d), { doctorId: 'doc1', dayOfWeek: 1, startTime: '09:00', endTime: '17:00' } as any);
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'availability.create' }));
  });
...
describe('Online-booking queue (Phase 22.4b)', () => {
  it('approve sets CONFIRMED/APPROVED and audits — only for pending', async () => {
    ...
    expect(d.onlineBooking.update).toHaveBeenCalledWith(expect.objectContaining({ data: { bookingStatus: 'CONFIRMED', approvalStatus: 'APPROVED' } }));
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'online_booking.approve' }));
  ...
  it('reject requires a reason, cancels the appointment, and audits', async () => {
    ...
    expect(d.appointment.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ status: 'CANCELLED', cancellationReason: 'Doctor unavailable that day' }) }));
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'online_booking.reject' }));
  });

  it('link-patient re-points booking + appointment to an existing patient and audits', async () => {
    d.onlineBooking.findFirst.mockResolvedValue({ id: 'b1', appointmentId: 'a1', patientId: 'auto1', possibleDuplicatePatient: true, duplicatePatientIds: [] });
    d.patient.findFirst.mockResolvedValue({ id: 'existing1', fullName: 'Asha' });
    await svc.linkBookingPatient(ctx(d), 'b1', 'existing1');
    expect(d.appointment.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'a1' }, data: { patientId: 'existing1' } }));
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'patient_record.linked_to_portal' }));
  });
...
describe('Document visibility & portal access (Phase 22.6)', () => {
  it('publish makes a document visible to the patient and audits', async () => {
    ...
    expect(d.patientDocument.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ visibleToPatient: true }) }));
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'document.publish_to_patient' }));
  });

  it('hide removes patient visibility with a reason and audits', async () => {
    d.patientDocument.findFirst.mockResolvedValue({ id: 'doc1', patientId: 'p1' });
    await svc.hideDocument(ctx(d), 'doc1', 'Uploaded in error');
    expect(d.patientDocument.update).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ visibleToPatient: false }) }));
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'document.hide_from_patient' }));
  });
  ...
  it('block portal access sets BLOCKED + audits', async () => {
    ...
    expect(d.patientPortalAccess.update).toHaveBeenCalledWith(expect.objectContaining({ data: { accessStatus: 'BLOCKED', blockReason: 'Suspected misuse' } }));
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'patient_portal_access.block' }));
  });

  it('revoke portal access sets REVOKED + audits', async () => {
    d.patientPortalAccess.findFirst.mockResolvedValue({ id: 'acc1' });
    await svc.revokePortalAccess(ctx(d), 'acc1', 'Patient requested removal');
    expect(d.patientPortalAccess.update).toHaveBeenCalledWith(expect.objectContaining({ data: { accessStatus: 'REVOKED', revokeReason: 'Patient requested removal' } }));
    expect(audit.log).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ action: 'patient_portal_access.revoke' }));
  });
});
