...
export const financeApi = {
  ...
  payments: (t: string, params: Record<string, string> = {}) => apiGet<FinancePaymentRow[]>(`/finance/payments${qs(params)}`, t),
  refunds: (t: string, params: Record<string, string> = {}) => apiGet<FinanceRefundRow[]>(`/finance/refunds${qs(params)}`, t),
  insuranceReceivables: (t: string) => apiGet<any[]>('/finance/insurance-receivables', t),
  dayClose: (t: string, params: Record<string, string> = {}) => apiGet<DayCloseSummary>(`/finance/day-close${qs(params)}`, t),
  ...
  requestApproval: (t: string, body: { type: string; entity: string; entityId?: string; amount?: number; reason: string; notes?: string }) =>
    apiPost<FinanceApproval>('/finance/approvals', body, t),
  approve: (t: string, id: string, reason: string) => apiPost<FinanceApproval>(`/finance/approvals/${id}/approve`, { reason }, t),
  reject: (t: string, id: string, reason: string) => apiPost<FinanceApproval>(`/finance/approvals/${id}/reject`, { reason }, t),
