...
export const publicAdminApi = {
  ...
  saveSettings: (t: string, body: Partial<PortalSettings>) => apiPut<PortalSettings>('/hms/patient-portal/settings', body, t),
  ...
  updateType: (t: string, id: string, body: Record<string, unknown>) => apiPatch<AppointmentType>(`/hms/appointment-types/${id}`, body, t),

  listDoctorProfiles: (t: string) => apiGet<DoctorProfileRow[]>('/hms/public-doctor-profiles', t),
  createDoctorProfile: (t: string, body: Record<string, unknown>) => apiPost<DoctorProfileRow>('/hms/public-doctor-profiles', body, t),
  publishDoctorProfile: (t: string, id: string) => apiPost<DoctorProfileRow>(`/hms/public-doctor-profiles/${id}/publish`, {}, t),
  hideDoctorProfile: (t: string, id: string, reason: string) => apiPost<DoctorProfileRow>(`/hms/public-doctor-profiles/${id}/hide`, { reason }, t),

  listAvailability: (t: string, doctorId: string) => apiGet<AvailabilityRule[]>(`/hms/availability-rules?doctorId=${doctorId}`, t),
  createAvailabilityRule: (t: string, body: Record<string, unknown>) => apiPost<AvailabilityRule>('/hms/availability-rules', body, t),
  updateAvailabilityRule: (t: string, id: string, body: Record<string, unknown>) => apiPatch<AvailabilityRule>(`/hms/availability-rules/${id}`, body, t),
