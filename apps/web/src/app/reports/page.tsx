...
function ReportsLanding() {
  ...
  const cards = useMemo(
    () =>
      [
        ...
        {
          show: modules.has('INVENTORY') && (perms.has('reports.inventory.read') || perms.has('inventory.reports.read') || perms.has('inventory.read')),
          title: 'Inventory',
  ...
  return (
    <>
      <PageHeader title="Reports" subtitle={data ? `Export-ready reports · updated ${formatDateTime(data.generatedAt)}` : 'Export-ready operational reports'} />
      {!data ? (
        <Spinner label="Loading report categories..." />
      ) : (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {cards.map((card) => (
            <ModuleCard key={card.href} title={card.title} value={card.value} hint={card.hint} href={card.href} badge={card.badge} />
          ))}
          {cards.length === 0 && (
            <ModuleCard title="No report access" value={<ClipboardList className="h-6 w-6" />} href="/dashboard" hint="Ask an administrator for report permissions." />
          )}
        </div>
      )}
    </>
...
export default function ReportsPage() {
  return (
    <Protected requireModule="REPORTS" allowedRoles={['HOSPITAL_ADMIN', 'HOSPITAL_MANAGER', 'ACCOUNTANT', 'BILLING', 'INVENTORY_MGR', 'DOCTOR', 'LAB_TECH']} requirePermission={['reports.read', 'reports.operational.read', 'reports.financial.read', 'reports.inventory.read', 'reports.clinical.read', 'bill.read', 'inventory.read', 'lab.read', 'encounter.read']}>
      <ReportsLanding />
    </Protected>
  );
}
