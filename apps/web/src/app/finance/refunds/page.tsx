...
import { financeApi, type FinanceRefundRow } from '@/lib/finance';
...
function RefundsPageInner() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setRows(await financeApi.refunds(t, params));
    } catch (e) {
                  ...
                  <tbody className="divide-y divide-line">
                    {rows.map((row) => (
                      <tr key={row.id} className="hover:bg-canvas">
                        ...
                        <td className="px-5 py-3 text-right font-medium text-danger-fg">-{money(row.amount)}</td>
...
export default function RefundsPage() {
  return (
    <Protected requireModule="BILLING" requirePermission={['payment.refund', 'finance.reconcile']}>
      <RefundsPageInner />
    </Protected>
  );
}
