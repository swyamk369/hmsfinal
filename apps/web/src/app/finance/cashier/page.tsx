...
import { Button, EmptyState, ErrorState, FormField, Input, Modal, PageHeader, Section, Select, Spinner, StatusChip } from '@/components/ui';
        ...
        <div className="space-y-6">
          ...
          <Section title="Find bill">
            <div className="grid gap-4 p-5 md:grid-cols-[1fr_auto]">
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search bill number, patient, or MRN" />
          ...
          {!rows ? (
            ...
            <EmptyState title="No unpaid or partial bills" hint="Bills from pending charges, OPD, pharmacy, IPD, or manual billing appear here when unpaid." />
          ) : (
            <Section title="Open bills">
              <BillTable bills={rows} />
              <div className="border-t border-line p-5">
                <Select value={payBill?.id ?? ''} onChange={(e) => setPayBill(rows.find((b) => b.id === e.target.value) ?? null)}>
...
function PaymentModal({ bill, onClose, onSaved }: { bill: Bill | null; onClose: () => void; onSaved: () => Promise<void> }) {
  const { activeTenantId } = useAuth();
  const toast = useToast();
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={submit} loading={busy} disabled={!bill || Number(amount) <= 0}>Collect</Button>
        </>
      }
    >
      {bill && (
        <div className="space-y-4">
          <div className="rounded-md border border-line bg-canvas px-3 py-2 text-body-sm">
            <div className="flex items-center justify-between">
              <Link href={`/finance/bills/${bill.id}`} className="font-mono text-primary">{bill.billNumber}</Link>
