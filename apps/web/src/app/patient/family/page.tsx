  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="mb-5 flex items-center justify-between">
        ...
        <button onClick={openAdd} className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-label-md font-medium text-white hover:bg-primary-700">
          <Plus className="h-4 w-4" /> Add member
        </button>
      </div>

      {members.length === 0 ? (
        <EmptyState icon={Users} title="No family members yet" body="Add a family member to manage their care and book appointments for them." />
      ) : (
          ...
          {members.map((m) => (
            <div key={m.id} className="flex items-start gap-3 rounded-xl border border-line bg-surface p-4">
              <span className="grid h-11 w-11 flex-shrink-0 place-items-center rounded-full bg-primary-100 text-label-md font-semibold text-primary-700">
                {m.fullName.split(/\s+/).slice(0, 2).map((w) => w[0]?.toUpperCase()).join('')}
              </span>
      ...
      <p className="mt-4 text-label-sm text-ink-soft">
        Booking for a family member creates their record at the hospital you choose. Cross-hospital record aggregation for dependents isn’t available yet.
      </p>
...
function FamilyForm({ member, onClose, onSaved }: { member: FamilyMember | null; onClose: () => void; onSaved: () => Promise<void> }) {
  const [form, setForm] = useState<FormState>(
    member ? { fullName: member.fullName, relationship: member.relationship, dob: member.dob?.slice(0, 10) ?? '', sex: member.sex ?? '', mobile: member.mobile ?? '' } : EMPTY,
  );
  ...
  return (
    <div className="fixed inset-0 z-30 grid place-items-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-md rounded-xl border border-line bg-surface p-6" onClick={(e) => e.stopPropagation()}>
        ...
        {error && <div className="mb-3 rounded-lg border border-danger/30 bg-danger-bg px-3 py-2 text-body-sm text-danger-fg">{error}</div>}
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              ...
              <select className={inputCls} value={form.relationship} onChange={(e) => setForm({ ...form, relationship: e.target.value })}>
        ...
        <div className="mt-5 flex justify-end gap-2">
          <button onClick={onClose} className="rounded-lg border border-line px-4 py-2 text-label-md font-medium text-ink hover:bg-canvas">Cancel</button>
          <button onClick={save} disabled={busy} className="rounded-lg bg-primary px-4 py-2 text-label-md font-medium text-white hover:bg-primary-700 disabled:opacity-50">
            {busy ? 'Saving…' : 'Save'}
          </button>
        </div>
      </div>
