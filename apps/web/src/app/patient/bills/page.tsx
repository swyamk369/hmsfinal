  ...
  return (
    <div>
      ...
      {rows.length === 0 ? (
        <EmptyState
          icon={Receipt}
          title={`No ${tab === 'all' ? '' : tab + ' '}bills`}
          body="Bills raised by your hospital appear here."
        />
      ) : (
            ...
            <div key={b.id} className="rounded-xl border border-line bg-surface p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-body-sm">
                <span className="text-ink-muted">
                  Total <span className="font-semibold text-ink">{portalMoney(b.netAmount)}</span>
                </span>
                <span className="text-ink-muted">
                  Paid <span className="font-medium text-ink">{portalMoney(b.paid)}</span>
                </span>
                <span className="text-ink-muted">
                  Due{' '}
                  <span className={`font-semibold ${b.due > 0 ? 'text-danger-fg' : 'text-ink'}`}>
                    {portalMoney(b.due)}
                  </span>
                </span>
              </div>
              ...
              {b.due > 0 && (
                <div className="mt-3 flex items-start gap-2 rounded-lg bg-canvas px-3 py-2 text-body-sm text-ink-muted">
                  ...
                  Online payment isn't available yet — please pay at the clinic or contact the hospital to settle this
                  bill.
                </div>
              )}
            </div>
